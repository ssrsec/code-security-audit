...
- **双模式审计**：redteam（围绕攻防演练得分点，按五个维度——应用权限获取 / 主机权限获取 / 数据库权限获取 / 数据获取 / 高危可组合漏洞——组织审计）/ full（redteam 全部 + 纯业务逻辑漏洞）
...
| 模式 | 关注范围 | 适用场景 |
|------|---------|---------|
| **redteam** | 围绕攻防演练得分点：应用/主机/数据库权限获取 + 敏感数据获取 + 高危可组合漏洞 | 渗透测试辅助、快速风险评估 |

...
阶段 4 验证时，根据漏洞类型查阅对应文档，结合项目版本/配置判断 **已确认/待验证/不成立**，并映射到 V0-V4 验证等级。**文档按漏洞类型与语言/框架扩展，不维护 SAST 规则库。**
...
| **A08** Software/Data Integrity | 不安全反序列化 | insecure_deserialization.md、fastjson_conditions.md 等 | 通用 + Java |
...
| 旧术语 | 当前语义 |
|--------|----------|
| CONFIRMED | 通常对应 V2/V3/V4；若仅代码静态闭环，则最多为 V1 待验证 |
| HYPOTHESIS | 对应 V1 待验证；缺少静态闭环时仍为 V0 候选线索 |
| Info/建议项 | 不进入漏洞表，可进入覆盖矩阵、总体建议或残余风险 |

知识库文档中的 `HYPOTHESIS/CONFIRMED` 是历史标记，实际输出必须以 `shared/verification_principles.md` 的 V0-V4 为准。

---

## 默认不纳入“真实危害”的类型（见 scope_policy）

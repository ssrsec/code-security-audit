...
阶段 4 验证时，根据漏洞类型查阅对应文档，结合项目版本/配置判断 **已确认/待验证**（映射：CONFIRMED/HYPOTHESIS）。**文档按漏洞类型与语言/框架扩展，不维护 SAST 规则库。**
...
- **强交互 CSRF**（用户必须点击/访问恶意页面才触发）→ 默认不进入最终报告，除非用户要求或满足例外。
- **A09 日志/监控**：多为加固建议，可标 Low/Info 或不单独建长篇文档。

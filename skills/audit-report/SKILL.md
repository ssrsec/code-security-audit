...
- **三个独立章节**（缺一即退回）：
  ...
  - 【实战利用】 场景数量按复杂度分级（复杂漏洞≥2 场景；简单漏洞 1 场景即可；待验证≥1 假设场景），详见 `shared/report_fields.md`
  - 【修复建议】 具体文件 + 代码示例 + 修复原理 + 业务影响评估 + 紧急临时方案/长期根本方案（架构级变更标注"需评估业务影响后执行"）
...
- **已确认**：复现步骤为 Burp 风格数据包且无违规占位、实战利用按复杂度分级（复杂漏洞≥2场景、简单漏洞1场景即可）且每场景含完整 Burp 数据包、反序列化给具体 gadget 链、注入给具体语句 → 全通过方可写入

...
1. 前置条件：（目标环境、账号权限说明；运行时变量获取方式，如 `{{access-token}}` 的获取步骤）

2. 使用以下安全复现证据验证漏洞（已确认漏洞：完整数据包；待验证漏洞：最佳努力 payload + 标注需运行时调整的部分）：
...
3. 预期安全行为：（正常应返回 401/403、拒绝参数、查询被参数化、命令不执行等）

4. 实际结果：（响应码、响应体关键字段、stdout/stderr、日志或测试断言）

5. 判定标准：（为什么该结果足以证明漏洞存在）

6. 清理步骤：（删除测试数据、回滚事务、撤销账号或说明无需清理）

# 安全审计报告

---

## 一、项目代码审计总结

| 项目 | 内容 |
|------|------|
| **审计目标** | {{ project_name }}（{{ project_path }}） |
| **开始时间** | {{ 读取 audit/phase0/metrics.md 中的阶段 0 开始时间 }} |
| **结束时间** | {{ 执行 bash date 命令获取当前时间填入 }} |
| **覆盖率** | 已审 {{ reviewed_count }} / 应审 {{ in_scope_count }} = {{ completion_pct }}% |
| **项目技术栈** | {{ tech_stack }} |

### 审计发现总结

- **漏洞总数**：{{ total_count }} 个
  - 严重：{{ critical }} 个
  - 高危：{{ high }} 个
  - 中危：{{ medium }} 个
  - 低危：{{ low }} 个
- **组合漏洞**：{{ chain_count }} 个
- **漏洞类型分布**：
  - （如：SQL 注入 X 个、RCE X 个、越权 X 个、未授权 X 个、文件操作 X 个...）
- **触发条件统计**：
  - 无需认证：{{ unauth_count }} 个
  - 普通用户认证：{{ low_priv_count }} 个
  - 管理员认证：{{ admin_count }} 个

---

## 二、漏洞汇总表

| 漏洞编号 | 漏洞名称 | 漏洞等级 | 验证状态 | 所需条件 |
|---------|---------|---------|---------|---------|
| vul-001 | （漏洞类型 + 关键条件） | 严重/高危/中危/低危 | 已确认/待验证 | 1. 前置条件：无<br>2. 访问权限：管理员 |

---

## 三、漏洞详情

### vul-001：（漏洞名称）

| 字段 | 内容 |
|------|------|
| **漏洞编号** | vul-001 |
| **漏洞名称** | 标准漏洞名称（不要拼接等级字段，不要用类似"GET /api/xxx..."这类不标准的名称） |
| **漏洞描述** | 简要说明漏洞原理和危害（不要引用过程文件）。 |
| **漏洞等级** | 严重 / 高危 / 中危 / 低危 |
| **验证状态** | 已确认 / 待验证 |
| **CVSS评分** | 完整向量字符串 `CVSS:3.1/AV:X/AC:X/PR:X/UI:X/S:X/C:X/I:X/A:X → X.X`，禁止使用"约" |
| **前置条件** | 具体前置条件（全报告术语必须统一，禁止敷衍描述） |
| **访问权限** | 具体所需的访问权限（禁止使用"视具体接口：多为已登录 Web 会话"这种敷衍描述，必须明确列出） |
| **调用链** | [入口文件:行号](相对路径) → [中转文件:行号](相对路径) → [Sink文件:行号](相对路径) |

<!-- 待验证漏洞须在表格中增加以下行（已确认漏洞删除此行）：
| **待验证内容** | ① 需验证：xxx；② 需验证：xxx |
-->

【复现步骤】

1. 前置条件：（目标环境、账号权限说明；运行时变量获取方式。**前置条件禁止循环论证 — 不得将漏洞利用效果作为前提，如"命令注入需要能执行命令"**）

2. 获取认证凭据（如需认证）：

```
POST /api/login HTTP/1.1
Host: 127.0.0.1:8080
Content-Type: application/json
Content-Length: 42

{"username":"test","password":"test123"}
```

响应中提取 token：`{"token":"eyJhbG..."}` → 用于后续请求的 Authorization 头

3. 使用以下 Burp 风格数据包复现漏洞（已确认漏洞：完整数据包；待验证漏洞：最佳努力 payload + 标注需运行时调整的部分）：

```
POST /api/vulnerable/endpoint HTTP/1.1
Host: 127.0.0.1:8080
Content-Type: application/json
Authorization: Bearer {{access-token}}
Content-Length: 52

{"param":"actual_payload_value","id":"12345"}
```

4. 预期安全行为：（正常应返回 401/403、拒绝参数、查询被参数化、命令不执行等）

5. 实际结果：（响应码、响应体关键字段、stdout/stderr、日志或测试断言）

6. 判定标准：（为什么该结果足以证明漏洞存在）

7. 清理步骤：（删除测试数据、回滚事务、撤销账号或说明无需清理）

【实战利用】

场景 1：（描述攻击者视角的完整利用路径，必须包含 Burp 风格数据包或可执行脚本）

```
POST /api/vulnerable/endpoint HTTP/1.1
Host: 127.0.0.1:8080
Content-Type: application/json
Authorization: Bearer {{access-token}}
Content-Length: 68

{"param":"specific_attack_payload_for_scenario_1"}
```

预期效果：（具体描述利用成功后攻击者获得的能力或数据）

场景 2：（高危/严重漏洞必须提供第二个不同攻击目标/深度的利用场景；待验证漏洞标注条件假设和运行时待确认点。同样必须包含完整 Burp 数据包。）

```
GET /api/another/attack/vector?param=payload HTTP/1.1
Host: 127.0.0.1:8080
Authorization: Bearer {{access-token}}

```

预期效果：（具体描述利用成功后的不同影响面）

影响边界：（说明该漏洞可造成的权限、数据、代码执行、内网访问、业务或组合链影响。不得引入阶段 4/5 未支撑的新漏洞。）

【修复建议】

**① 紧急临时方案（快速止血）**：
（立即可执行的最小修复措施，如禁用接口、增加鉴权中间件、配置限制等）

**② 长期根本方案（彻底修复）**：
（具体修改哪个文件、代码示例、修复原理、回归测试建议）

**业务影响评估**：
（说明修复方案是否影响正常业务功能。如涉及架构级变更，标注"需评估业务影响后执行"。如修复会中断业务流，提供兼顾安全和业务的过渡方案。）

---

（重复以上结构，每条漏洞一个章节）

---

## 四、组合漏洞摘要

### 4.1 漏洞组合攻击链（基于已确认漏洞）

#### 【组合漏洞汇总表】

| 组合漏洞编号（必须由 ≥2 个已知单漏洞组合而成，单独的一个漏洞不能作为组合漏洞） | 涉及漏洞组合 | 攻击场景描述 | 组合后漏洞等级 |
|----------------------------------------------------------------------|------------|------------|--------------|
| comp-001 | vul-XXX + vul-XXX | （简述攻击过程） | 严重/高危 |

#### 【组合漏洞详情】

##### comp-001

**组合**：vul-XXX（名称）+ vul-XXX（名称）
**漏洞等级（组合后）**：严重
**攻击链**：
1. 先通过 vul-XXX 的（漏洞名称），实现/拿到（什么能力/信息）
2. 然后利用 vul-XXX 的（漏洞名称），完成（最终危害）

---

（若无组合漏洞则写：`> 经分析，未发现可组合利用的漏洞链。`）

---

### 4.2 原语组合攻击链（基于能力片段推导）

来源：`audit/phase5/primitive_chains.md`

| 编号 | 攻击链名称 | 参与原语 | 推导等级 | 置信度 |
|------|-----------|---------|---------|--------|
| chain-001 | 受限写+Cron劫持→RCE | prim-001+prim-007 | 高危 | ✓ 已确认 |
| chain-002 | SSRF+云凭证→密钥窃取 | prim-003+prim-011 | 高危 | ⚠ 待验证 |

（若无原语攻击链则写：`> 经原语组合分析，未发现可组合的原语攻击链。`）

---

## 五、总体安全建议

### 架构层面
（根据审计发现给出架构级安全建议）

### 开发流程
（安全开发相关建议）

### 高频漏洞修复
（针对本次审计中出现频率较高的漏洞类型，给出统一修复方案）

<!-- 报告到此结束。严禁在第五章节之后添加任何内容（如"报告说明"、"免责声明"、"附录"等）。 -->

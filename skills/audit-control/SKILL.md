...
读取 `audit/phase0/config.json` 中的 `mode` 字段。

### mode = "redteam"

按攻防演练得分点维度审计，聚焦权限获取和数据获取：

**完整执行**（安全控制类）：
- D2 认证（应用权限获取）：登录绕过、JWT 伪造/签名不校验、Session 固定、弱口令/默认凭证、验证码绕过、密码重置缺陷、SSO/OAuth 配置错误
- D3 授权（权限边界突破）：未授权访问高危接口、垂直越权获取管理员权限、水平越权导致大规模数据泄露、IDOR/BOLA 突破权限边界
- D8 配置（信息泄露与入口暴露）：Actuator/调试端点未授权、硬编码凭据可直接利用、敏感信息泄露接口

**跳过**（纯业务逻辑缺陷 — 安全控制正常但业务规则有漏洞）：
- D9 中的业务逻辑部分：支付金额篡改、订单/审批状态机跳跃、优惠券/积分/库存规则绕过
- 竞态条件下的业务重复操作（但竞态条件导致 getshell 或权限获取的仍属红队范围）
- D3 中的业务级水平越权：查看他人地址/订单详情（未导致大规模数据泄露）
- Mass Assignment 修改业务字段（如修改自己订单的价格）
- 需要用户交互触发的漏洞：CSRF、反射型/DOM XSS（存储型 XSS 能直接获取管理员权限的除外）

**灵活判定**：同一漏洞类型根据实际危害决定是否属于红队范围。例如：
- 文件上传条件竞争 → 可能 getshell → 属于红队模式（主机权限获取）
- 优惠券条件竞争 → 业务损失 → 不属于红队模式
...
- finding id
- **漏洞名称**：必须以标准漏洞类型开头（如 未授权访问、权限校验绕过、越权操作），括号补充关键位置或条件，使用业务语义描述。禁止直接用代码类名/方法名做名称（如"WFOMPermissionImpl.hasPermission 恒为 true"应改为"权限校验绕过（hasPermission 恒返回 true）"）。

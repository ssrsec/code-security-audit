...
2. **调用链追踪器（含污点节点类型）**：跨 2 个以上文件的数据流必须将每一跳写入 `audit/phase2/callchain_tracker.md`。**每一跳必须按 `shared/taint_propagation.md` §1 节点分类标记**（`[S]`/`[P]`/`[T]`/`[X]`/`[B]`/`[K]`）；遇到 sanitizer 必须按 §2 四问判别真伪；遇到 base64/urlencode 等变换必须按 §3 不消污规则不要误判为已清污；遇到 if/switch 分支必须按 §4 对每个分支独立标污点。格式：

```
## cc-001: [Sink 类型]
R1-R7 可达性  ✓✓✓✓✗✓✓     ← 按 sink_reachability_checklist.md 7 步打勾
[S] UserController.java:45  — @RequestParam String userId             taint=initial
[P] UserService.java:102    — findUser(id) 参数传递                   taint=preserved
[B] UserDao.java:31  if (id.matches("^[0-9]+$"))
  ├─[X]TRUE  parameterizedQuery(id)                                   taint=cleared
  └─[P]FALSE legacy("... WHERE id=" + id)                             taint=preserved
[K] UserDao.java:33         — Statement.executeQuery(sql)             ✓ sink_reached
判定：FALSE 分支可达 → 候选成立（绕过点：可输入非数字 ID）
```
...
**对每条候选必须执行 R1-R7 可达性检查（按 `shared/sink_reachability_checklist.md` §1）：**

- R1 sink 定义存在 / R2 sink 被实际调用 / R3 调用点被生产路径加载 / R4 路由对外可达 / R5 全局拦截是否覆盖且不可绕过 / R6 source 真正可控 / R7 运行时前提（gadget/JDK 版本等）满足

7 步打勾结果必须写入 `callchain_tracker.md` 顶部。R5 发现绕过 → 优先级提升；R2/R3 不通过 → 排除（写负证据）；R7 不通过 → 标待验证（不丢弃）。防护是否最终有效留给阶段 4 验证，**但阶段 2 必须记录 7 步打勾**。

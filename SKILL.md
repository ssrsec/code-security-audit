...
**判定原则**：两种模式的唯一区别在于**是否审计纯业务逻辑漏洞**。安全控制类漏洞（认证、授权、注入、执行等）在两种模式下都审。

**redteam 模式**：审计所有突破安全控制的漏洞，跳过纯业务逻辑缺陷

核心判定问题：「攻击者能否利用此漏洞获取未授权的系统访问、执行代码、或窃取敏感数据？」
- 答案为是 → redteam 关注
- 漏洞本质是业务规则设计缺陷而非安全控制缺失 → redteam 不关注

**redteam 关注的范围**（只要突破了安全控制边界）：
- 代码执行类：RCE、命令注入、反序列化、SSTI、表达式注入、文件上传 getshell
- 注入类：SQL 注入、NoSQL 注入、LDAP 注入、XPath 注入
- 认证绕过类：登录绕过、JWT 伪造/签名不校验、Session 固定、弱口令/默认凭证、验证码绕过、密码重置缺陷
- 授权突破类：未授权访问高危接口、垂直越权、水平越权导致大规模数据泄露
- 数据窃取类：任意文件读取、敏感信息泄露（凭证/密钥/PII）、目录遍历
- 服务端请求类：SSRF（可访问内网/云元数据）
- 供应链/已知漏洞：依赖组件已知高危 CVE

**redteam 不关注**（纯业务逻辑缺陷 — 安全控制正常工作，但业务规则有漏洞）：
- 支付金额篡改、0 元购、负数退款
- 订单/审批状态机跳跃（未支付→已发货）
- 优惠券/积分/库存的业务规则绕过
- 竞态条件下的业务重复操作（非安全控制绕过的竞态）
- Mass Assignment 修改业务字段（如修改自己订单的价格）
- 业务级水平越权（查看他人地址/订单详情，但未导致大规模数据泄露）

**full 模式**：redteam 全部 + 所有业务逻辑漏洞
- 包含 redteam 不关注的全部业务逻辑缺陷
- 适用于需要全面评估业务安全性的场景

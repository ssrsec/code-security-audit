...
**只有在确认 MCP 工具确实不可用后**（通过 2.0 节的检测流程），才执行以下降级：
1. **必须明确告知用户**："当前环境未检测到反编译 MCP 工具（`java-decompiler` / `ilspy-mcp`），无法对编译产物进行审计。请确认 MCP 服务已正确配置并重启会话后重试。"
2. **严禁自行下载或安装反编译工具**（如通过 `curl`/`wget` 下载 CFR JAR、通过 `pip`/`brew` 安装工具）
3. **严禁**在无法反编译的情况下假装审计或跳过编译产物
...
- 阶段 6 的清理命令只能是 `rm -rf audit/phase0 audit/phase1 audit/phase2 audit/phase3`，不得包含 `audit/decompiled`

...
| 平台 | 调度方式 |
|------|----------|
| CC | 同一消息中 `Agent("audit-validate-agent", ...)` + `Agent("audit-composer-agent", ...)` |
| Cursor | 同一消息中两个 `Task(subagent_type="generalPurpose", ...)` |
| 降级 | 先执行 validate，再执行 composer（串行） |
...
每个 Agent YAML 头部包含 `model_tier` 字段，声明推荐的计算层级。orchestrator 在调度时根据此字段选择模型。

### 层级定义

| model_tier | 含义 | 推荐场景 | CC 示例 | Cursor 行为 |
|------------|------|---------|---------|-------------|
| fast | 快速推理，低成本 | 侦察、报告生成等结构化输出 | haiku/sonnet | 忽略（inherit） |
| balanced | 中等推理 | 编排、状态管理 | sonnet | 忽略（inherit） |
| high | 深度推理 | Sink/Control 追踪、原语组合 | sonnet-max-thinking | 忽略（inherit） |
| max | 最高精度 | 漏洞验证、CVSS 评分 | opus-thinking | 忽略（inherit） |

### 各 Agent 路由配置

| Agent | Phase | model_tier | 理由 |
|-------|-------|------------|------|
| audit-orchestrator | 全程 | balanced | 编排逻辑无需深度推理 |
| audit-recon-agent | Phase 1 | fast | 侦察以文件枚举和模式匹配为主 |
| audit-sink-agent | Phase 2 | high | 数据流追踪需要深度代码推理 |
| audit-control-agent | Phase 2 | high | 鉴权分析需要理解业务语义 |
| audit-advisor-agent | Phase 2 | balanced | 监督纠偏以宏观判断为主 |
| audit-validate-agent | Phase 4+5 | max | 验证是最终质量关卡，需要最高精度 |
| audit-composer-agent | Phase 5 | high | 原语组合需要跨文件推理 |
| audit-report-agent | Phase 6 | fast | 报告生成以模板填充为主 |
...
TIER_TO_MODEL = {
    "fast": "haiku",
    "balanced": "sonnet",
    "high": "sonnet",       # 可升级为 opus
    "max": "opus",
}

agent_tier = read_agent_yaml(agent_name)["model_tier"]
model = TIER_TO_MODEL.get(agent_tier, "sonnet")
Agent(name=agent_name, model=model, prompt=...)
```

### Cursor 模式

Cursor 不支持在 Task 调度中指定模型，`model_tier` 仅作为文档性声明。未来 Cursor 支持模型选择后可自动生效。

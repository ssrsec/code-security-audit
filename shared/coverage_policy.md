...
> **本文件职责边界**
> - **单一事实来源**：三层覆盖率定义（文件枚举/静态扫描/深读）、10 个安全审计维度（D1-D10）、双轨对应关系、阶段门标准
> - **不覆盖**：审计范围（什么算漏洞） → `audit_discipline.md`；报告格式 → `report_fields.md`；可达性判定 → `sink_reachability_checklist.md`
> - **关联**：`large_project_audit.md`（大项目分批策略）、`coverage_matrix_template.md`（覆盖矩阵模板）
...
- 若平台或上下文限制导致不能继续，写入 `audit/state.json`（含 `contextSummary`，见 `shared/state_schema.md` §6）并提示用户继续，不得提前生成最终报告。
- **反偷懒阶段门**：每轮执行必须满足 `shared/large_project_audit.md` §8 的最低批次工作量，未达下限禁止中断。

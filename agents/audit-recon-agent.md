...
判断审计目标是否为已知开源系统或通用产品（如 RuoYi、Jeecg、WordPress、Jenkins 等）。检查线索：
- README、构建文件中的项目名和包名
- 特征性目录结构和配置（如 `com.ruoyi`、`wp-content/`、`JENKINS_HOME`）
- 版本号标记

**识别到已知系统时**（按 `skills/audit-recon/SKILL.md` 3.2 节执行）：
1. 联网搜索该系统+版本的已知 CVE、安全分析文章、公开 PoC、默认凭证
2. 整理为结构化情报写入 `audit/phase1/known_system_intel.md`
3. 提取高价值审计线索（重点端点、已知攻击面、已知绕过手法）

未识别到已知系统时，在 `known_system_intel.md` 写空即可。
...
| `audit/phase1/known_system_intel.md` | 已知系统历史漏洞情报 |
...
输入：audit/phase0/metrics.md、audit/phase0/config.json、项目根路径 {project_path}
输出：audit/phase1/ 下的全部产出（project_inventory.json、auth_model.md、endpoint_list.md、sink_list.md、in_scope_files.txt、tier_list.json、coverage_matrix.md、known_system_intel.md 等）
规则：文件路径必须 Glob/Read 验证；代码片段只来自 Read 输出；识别到已知通用系统时必须联网搜索历史漏洞和安全分析文章写入 known_system_intel.md；所有输出使用简体中文。
```

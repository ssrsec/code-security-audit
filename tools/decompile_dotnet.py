#!/usr/bin/env python3
"""Run .NET decompilation tasks from decompilation_plan.json.

Default mode is dry-run. Use --execute to call the repository's ilspycmd wrapper.
"""
...
UNIX_WRAPPER = ROOT / "scripts" / "tools" / "decompilers" / "bin" / "decompile-dotnet.sh"
WINDOWS_WRAPPER = ROOT / "scripts" / "tools" / "decompilers" / "bin" / "decompile-dotnet.cmd"
...
def is_dotnet_task(task: dict[str, Any]) -> bool:
    assets = task.get("recommendedAssets") or []
    return task.get("platform") in {"dotnet", "native-or-dotnet"} or any("dotnet" in str(asset) for asset in assets)


def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Run .NET decompilation tasks")
